...
var JetpackFile = function(jp, path_) {
   ...
   */
  self.render = async function(argv) {
    const transform = transforms[_.last(exts) || 'none'] || transforms.none;
    const content   = await transform(fullpath, ext, argv);
  ...
  transforms.none = async function(file, ext, argv) {
    if (ext === 'json') {
  ...
  transforms.js = async function(file, ext, argv) {
