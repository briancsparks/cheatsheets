...
var JetpackFile = function(jp, path_) {
   ...
   */
  self.render = async function(...args) {
    const transform = transforms[_.last(exts) || 'none'] || transforms.none;
    const content   = await transform(fullpath, ext, ...args);
  ...
  transforms.none = async function(file, ext, ...args) {
    if (ext === 'json') {
  ...
  transforms.js = async function(file, ext, ...args) {
